...
class j16000showplugins
{
	public function __construct()
	{
		...
		if (!isset($key_validation->allowed_plugins)) {  // If the allowed plugins variable isn't set then it's an old cache of license data, let's force a refetch
		...
		foreach ($remote_plugins as $rp) {
			...
			if (array_key_exists($rp[ 'name' ], $installed_plugins)) {
				...
				if (!$rp[ 'retired' ]) {
					//$r[ 'UNINSTALL' ] = '<button type="button" href="'.$r[ 'UNINSTALL_LINK' ].'" class="btn btn-danger" >'.$uninstall_text.'</button>';
					...
					$r[ 'UNINSTALL' ] = '<button type="button"  href="'.$r[ 'UNINSTALL_LINK' ].'" class="btn btn-success" >'.$uninstall_text.'</button>';
				}
			}
			...
			if ($condition == 1 && (array_key_exists($rp[ 'name' ], $current_licenses) || $developer_user)) {
				if (using_bootstrap()) {
					...
					$r[ 'INSTALL' ] = '<button href="'.$r[ 'INSTALL_LINK' ].'" class="fg-button ui-state-default ui-corner-all "   >'.$r[ 'INSTALL_TEXT' ].'</button>';
				}
			}

			if ( $key_validation->allowed_plugins[0] != "*" && !in_array( $rp[ 'name' ] , $key_validation->allowed_plugins ) ) {
				$r[ 'INSTALL' ] = '<button type="button"  class="btn btn-primary disabled"  >'.$r[ 'INSTALL_TEXT' ].'</button>';
			}
			...
			if (!isset($jrConfig[ 'cmf_license_key' ])) {
				$jrConfig[ 'cmf_license_key' ] = '';
			}
		...
		if (jomres_bootstrap_version() == 4 ) {
			$tmpl->readTemplatesFromInput('plugin_manager_bs4.html');
		} else {
