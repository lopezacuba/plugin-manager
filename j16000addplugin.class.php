...
class j16000addplugin
{
	public function __construct($componentArgs)
	{
		...
		/* if ( substr($pluginName, 0, 17) == "channelmanagement" || $pluginName == 'api_feature_cmf' ) { 
			if ( !isset($jrConfig[ 'cmf_license_key' ]) || trim($jrConfig[ 'cmf_license_key' ]) == '' ) {
				$user_allowed_to_download = false;
			} else {
				$key_to_send = $jrConfig[ 'cmf_license_key' ];
			}
		} */
